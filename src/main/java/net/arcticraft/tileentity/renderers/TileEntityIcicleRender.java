	...
	public void renderAModelAt(TileEntityIcicle tileEntity, double x, double y, double z, float f)
	{
		...
		if(icicle != null)
		{
			...
			if(icicle.upsideDown){
				...
				GL11.glTranslatef(0, (float) -1.0F, 0);
				
				if(icicle.blockType != null)
				{
					icicle.blockType.setBlockBounds(0, 0.5f, 0, 1, 2.0F, 1);
				}
			}
			else
			{
				if(icicle.blockType != null)
				{
					icicle.blockType.setBlockBounds(0, 0, 0, 1, 1.5F, 1);
				}
			}
		}
		else
