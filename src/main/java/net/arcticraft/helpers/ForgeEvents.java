...
public class ForgeEvents{
	...
	public void playerInteract(PlayerInteractEvent event)
	{
 	...
 	/*@SubscribeEvent
	public void pourBucket(PlayerUseItemEvent.Tick event)
	{
		MovingObjectPosition position = Minecraft.getMinecraft().objectMouseOver;
		// Block block = event.entityPlayer.worldObj.getBlock(position.blockX, position.blockY, position.blockZ);
		System.out.println("call");
		if(event.entityPlayer.dimension == 3 && event.item.getItem() == Items.water_bucket)
		{
			event.entityPlayer.worldObj.setBlock(position.blockX, position.blockY, position.blockZ, ACBlocks.frostWaterIce);
		}
	}*/
}
