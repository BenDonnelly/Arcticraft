...
public class ACChestGenHooks extends ChestGenHooks
{
    //Currently implemented categories for chests/dispensers, Dungeon loot is still in DungeonHooks
    public static final String AC_CHEST_BASIC       = "acChestBasic";

    private static final HashMap<String, ACChestGenHooks> chestInfo = new HashMap<String, ACChestGenHooks>();
    ...
    private static void init()
    {
        ...
        addInfo(AC_CHEST_BASIC,       Arcticraft.acChestContent,   3,  7);
    }

    static void addDungeonLoot(ACChestGenHooks dungeon, ItemStack item, int weight, int min, int max)
    {
    ...
    {
        chestInfo.put(category, new ACChestGenHooks(category, items, min, max));
    }
     ...
     */
    public static ACChestGenHooks getInfo(String category)
    {
        if (!chestInfo.containsKey(category))
        {
            chestInfo.put(category, new ACChestGenHooks(category));
        }
    ...
    public ACChestGenHooks(String category)
    {
    	super(category);
    ...
    public ACChestGenHooks(String category, WeightedRandomChestContent[] items, int min, int max)
    {
        this(category);
    ...
    public ItemStack getOneItem(Random rand)
    {
        ...
        ItemStack[] stacks = ACChestGenHooks.generateStacks(rand, item.theItemId, item.theMinimumChanceToGenerateItem, item.theMaximumChanceToGenerateItem);
