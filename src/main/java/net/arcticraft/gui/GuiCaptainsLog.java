...
public class GuiCaptainsLog extends GuiScreen{

	// Actually not implemented.
	private static final int maxPages = 100;
	private static final ResourceLocation texture = new ResourceLocation(Arcticraft.MOD_ID, "textures/gui/captains_log.png");
	...
	private GuiCaptainLogButtonNextPage buttonNextPage;
	private GuiCaptainLogButtonNextPage buttonPreviousPage;

	public GuiCaptainsLog(){}
	...
	public void drawScreen(int par1, int par2, float par3)
	{
		...
		this.fontRendererObj.drawString(s, x + (this.bookImageWidth - this.fontRendererObj.getStringWidth(s) - 10) / 2, y + 158, 4210752);
		int i = y + this.indentY;
		if(this.currPage != 1)
		{
			for(Iterator iterator = textForPage[this.currPage].iterator(); iterator.hasNext(); i += this.fontRendererObj.FONT_HEIGHT)
			{
				String s1 = (String) iterator.next();
				this.fontRendererObj.drawSplitString(s1, x + this.indentX, i, this.writablePageWidth, 0x111111);
			}
		}
		else
		{
			...
			this.fontRendererObj.drawString(title1, x + (this.bookImageWidth - this.fontRendererObj.getStringWidth(title1)) / 2, i + 10, 0x111111);
			this.fontRendererObj.drawString(title2, x + (this.bookImageWidth - this.fontRendererObj.getStringWidth(title2)) / 2, i + 20, 0x111111);
		}
	...
	{
		int pageNumber = 2;
		int linesPerPage = (int) Math.floor((double) this.writablePageHeight / (double) this.fontRendererObj.FONT_HEIGHT);
		for(String[] log : logs)
		{
			...
			for(int i = 0; i < log.length; i++)
			{
				...
				lines.addAll(this.fontRendererObj.listFormattedStringToWidth(text, this.writablePageWidth));
			}
		...
		int x = (this.width - this.bookImageWidth) / 2;
		int y = (this.height - this.bookImageHeight) / 2;
		this.buttonList.add(this.buttonNextPage = new GuiCaptainLogButtonNextPage(0, x + 120, y + 154, true));
		this.buttonList.add(this.buttonPreviousPage = new GuiCaptainLogButtonNextPage(1, x + 38, y + 154, false));
		this.updateButtons();
	}
	...
	{
		this.buttonNextPage.enabled = this.currPage < this.bookTotalPages;
		this.buttonPreviousPage.enabled = this.currPage > 1;
	}
