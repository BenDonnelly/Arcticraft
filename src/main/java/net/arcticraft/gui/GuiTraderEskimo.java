...
public class GuiTraderEskimo extends GuiScreen{
	...
	public void initGui()
	{
		...
		for(int ID = 0; ID < trades.length; ID++)
		{
			...
			if(trade != null)
			{
				...
				this.buttonList.add(new GuiTraderButton(ID, x, y, trade, this));
			}
		}
	}
	...
	public void actionPerformed(GuiButton button)
	{
		if(button.enabled)
		{
			...
			if(trade != null && this.inventory.getFirstEmptyStack() != -1)
			{
				...
				/*
				 * ByteArrayOutputStream bos = new ByteArrayOutputStream(16); DataOutputStream outputStream = new DataOutputStream(bos);
				 * 
				 * try { outputStream.writeInt(Item.getIdFromItem(item)); outputStream.writeInt(stackSize); outputStream.writeInt(damageValue); outputStream.writeInt(gems); } catch (Exception ex) { ex.printStackTrace(); }
				 * 
				 * S3FPacketCustomPayload packet = new S3FPacketCustomPayload("AC|EskimoTrade", bos.toByteArray());
				 * 
				...
				/*
				 * if (this.inventory.player instanceof EntityClientPlayerMP) { EntityClientPlayerMP player = (EntityClientPlayerMP) this.inventory.player;
				 * 
	...
	public void drawScreen(int par1, int par2, float par3)
	{
		...
		FMLClientHandler.instance().getClient().renderEngine.bindTexture(new ResourceLocation(Arcticraft.MOD_ID, "textures/gui/trading.png"));
		...
		String gems = EskimoTrade.getGemsFromInventory(this.inventory) + " Gems";
