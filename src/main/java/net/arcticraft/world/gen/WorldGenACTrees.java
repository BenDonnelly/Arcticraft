...
public class WorldGenACTrees implements IWorldGenerator{
	...
	public static WorldGenACTrees[] possibles = new WorldGenACTrees[] {new WorldGenACTrees(6, 4, 5, 4, 8, 5, 0.7F, 0.6F),
	...
	public WorldGenACTrees(int height, int extensionHeight, int boughAmount, int boughSize, int boughSpread, int branchVariation, float resetRate, float resetToLogRate){
		this.height = height;
	...
	{
		if(world.provider.dimensionId == 0) // Only generate in the overworld
		{
			for(int i = 0; i < random.nextInt(5); i++)
			{
				...
				while(y > 0 && world.getBlock(x, y - 1, z) != Blocks.grass)
					y -= 1;

				if(world.getBlock(x, y - 1, z) == Blocks.grass && world.getBlock(x + 1, y - 1, z) == Blocks.grass && world.getBlock(x + 1, y - 1, z + 1) == Blocks.grass && world.getBlock(x, y - 1, z + 1) == Blocks.grass)
				{
			...
			for(int i = 0; i < 5; i++)
			{
				...
				{
					new WorldGenFrostTree(false, 4, 0, 0, false).generate(world, random, x, y, z);
					System.out.println("frost tree spawned:" + x + "|" + y + "|" + z);
				}
			}
		}
	}	
	...
	public WorldGenACTrees(int height, int extensionHeight, int boughAmount, int boughSize, int boughSpread, int branchVariation, float resetRate, float resetToLogRate, Block log, Block leaves){
		this(height, extensionHeight, boughAmount, boughSize, boughSpread, branchVariation, resetRate, resetToLogRate);
		this.log = log;
		this.leaves = leaves;
	}

	public WorldGenACTrees(int height, int extensionHeight, int boughAmount, int boughSize, int boughSpreadX, int boughSpreadY, int boughSpreadZ, int branchVariation, float resetRate, float resetToLogRate){
	...
	public WorldGenACTrees(int height, int extensionHeight, int boughAmount, int boughSize, int boughSpreadX, int boughSpreadY, int boughSpreadZ, int branchVariation, float resetRate, float resetToLogRate, Block log, Block leaves){
	...
	private static final int[][] smallBoughCoords = new int[][] { {0, 1}, {0, 2}, {1, 1}, {1, 2}, {2, 1}, {2, 2}, {3, 1}, {3, 2},
	...
	public void generateTree(World world, Random random, int x, int y, int z)
	{
		...
		// Random roots
		/*
		 * for(int x2 = -1; x2 < 3; x2++) { for(int z2 = -1; z2 < 3; z2++) { Block b = world.getBlock(x2, y, z2); if((b == Blocks.air || b.isReplaceable(world, x2, y, z2)) && random.nextBoolean()) { world.setBlock(x2, y, z2, log, logMeta, 2);
		...
		{
			Vector3f boughLocation = new Vector3f(boughStart.x + random.nextInt(boughSpreadX) - boughSpreadXBase, boughStart.y + logExtension + random.nextInt(boughSpreadY) - boughSpreadYBase, boughStart.z + random.nextInt(boughSpreadZ) - boughSpreadZBase);
			...
			for(int x2 = (int) (boughLocation.x - boughRadius); x2 < boughLocation.x + boughRadius; x2++)
			{
				for(int y2 = (int) (boughLocation.y - boughRadius); y2 < boughLocation.y + boughRadius; y2++)
				{
					...
					{
						if(world.getBlock(x2, y2, z2) == Blocks.air && VectorUtils.distance(boughLocation, x2, y2, z2) < boughRadius)
						{
			...
			while(Math.abs(branch.x - boughLocation.x) > 1 || Math.abs(branch.y - boughLocation.y) > 1 || Math.abs(branch.z - boughLocation.z) > 1)
			{
				if(branch.x < boughLocation.x)
					branch.x++;
				else if(branch.x > boughLocation.x) branch.x--;

				if(branch.y < boughLocation.y)
					branch.y++;
				else if(branch.y > boughLocation.y) branch.y--;

				if(branch.z < boughLocation.z)
					branch.z++;
