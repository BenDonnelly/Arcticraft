	...
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator, IChunkProvider chunkProvider)
	{
		if(world.provider.dimensionId == 0) // Only generate in the overworld
		{
			for (int i = 0; i < random.nextInt(5); i++)
			{
				...
				while(y > 0 && world.getBlock(x, y - 1, z) != Blocks.grass) y -= 1;
				
				if(world.getBlock(x, y - 1, z) == Blocks.grass
						&& world.getBlock(x + 1, y - 1, z) == Blocks.grass
						&& world.getBlock(x + 1, y - 1, z + 1) == Blocks.grass
						&& world.getBlock(x, y - 1, z + 1) == Blocks.grass)
				{
