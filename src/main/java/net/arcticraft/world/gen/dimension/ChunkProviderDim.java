...
public class ChunkProviderDim implements ITempComponent, IChunkProvider {
	...
	public ChunkProviderDim(World p_i2006_1_, long p_i2006_2_,
			boolean p_i2006_4_) {
		...
		NoiseGenerator[] noiseGens = { field_147431_j, field_147432_k,
				...
				mobSpawnerNoise };
		noiseGens = TerrainGen.getModdedNoiseGenerators(p_i2006_1_, this.rand,
				noiseGens);
	...
	public void generateArcanestone(World p_76484_1_, Random p_76484_2_,
			int p_76484_3_, int p_76484_4_, int p_76484_5_, float veinSize) {
		p_76484_1_.setBlock(p_76484_3_, p_76484_4_, p_76484_5_,
				ACBlocks.arcaneStone, 0, 2);
	...
	public void replaceBlocksForBiome(int par1, int par2, Block[] blocks,
		...
		ChunkProviderEvent.ReplaceBiomeBlocks event = new ChunkProviderEvent.ReplaceBiomeBlocks(
				this, par1, par2, blocks, par3ArrayOfByte,
		...
		this.stoneNoise = this.field_147430_m.func_151599_a(this.stoneNoise,
				(double) (par1 * 16), (double) (par2 * 16), 16, 16, d0 * 2.0D,
				d0 * 2.0D, 1.0D);

		for (int k = 0; k < 16; ++k) {
			for (int l = 0; l < 16; ++l) {
				ACBiomeGenBase biomegenbase = (ACBiomeGenBase) par4ArrayOfBiomeGenBase[l
						+ k * 16];
				biomegenbase.genTerrainBlocks(this.worldObj, this.rand, blocks,
						par3ArrayOfByte, par1 * 16 + k, par2 * 16 + l,
	...
	public Chunk provideChunk(int p_73154_1_, int p_73154_2_) {
		this.rand.setSeed((long) p_73154_1_ * 341873128712L + (long) p_73154_2_
				* 132897987541L);
		...
		this.replaceBlocksForBiome(p_73154_1_, p_73154_2_, ablock, abyte,
				this.biomesForGeneration);
		this.caveGenerator.func_151539_a(this, this.worldObj, p_73154_1_,
				p_73154_2_, ablock);
		...
		Chunk chunk = new Chunk(this.worldObj, ablock, abyte, p_73154_1_,
				p_73154_2_);
			...
			for (int k1 = 0; k1 < 5; ++k1) {
				...
				for (int l1 = -b0; l1 <= b0; ++l1) {
					for (int i2 = -b0; i2 <= b0; ++i2) {
						...
						if (this.field_147435_p == WorldType.AMPLIFIED
								&& f3 > 0.0F) {
							f3 = 1.0F + f3 * 2.0F;
							f4 = 1.0F + f4 * 4.0F;
						}
	...
	public void populate(IChunkProvider p_73153_1_, int p_73153_2_,
		...
		if (biomegenbase != ACBiomeGenBase.glacier
				&& biomegenbase != ACBiomeGenBase.frostOcean
				&& biomegenbase != ACBiomeGenBase.snowPlains) {
			new WorldGenACTrees().generate(rand, p_73153_2_, p_73153_3_,
					this.worldObj, p_73153_1_, p_73153_1_);
		}
		...
		for (int i = 0; i < 3; i++) {
			...
			(new WorldGenMineable(ACBlocks.tekkiteOre, 5, 0)).generate(
					worldObj, rand, x, y, z);
		...
		for (int i = 0; i < 5; i++) {
			...
			int y = rand.nextInt(256);
			int z = l + rand.nextInt(16);
			(new WorldGenMineable(ACBlocks.escariaOre, 6, 0)).generate(
					worldObj, rand, x, y, z);
		...
		for (int i = 0; i < 7; i++) {
			...
			int y = rand.nextInt(256);
			int z = l + rand.nextInt(16);
			(new WorldGenMineable(ACBlocks.glacianOre, 7, 0)).generate(
					worldObj, rand, x, y, z);
		...
		for (int i = 0; i < 10; i++) {
			...
			int y = rand.nextInt(256);
			int z = l + rand.nextInt(16);
			(new WorldGenMineable(ACBlocks.rigentemOre, 12, 0)).generate(
					worldObj, rand, x, y, z);
		...
		for (int i = 0; i < 7; i++) {
			...
			int y = rand.nextInt(256);
			int z = l + rand.nextInt(16);
			(new WorldGenMineable(ACBlocks.eriumOre, 8, 0)).generate(worldObj,
					rand, x, y, z);
		...
		for (IGenComponent tmp : genList) {
			if (tmp != null) {
				...
				if (tmp.bottomBlocks.contains(this.worldObj.getBlock(x,
						y + tmp.heightFromGround, z).getClass())
						&& this.worldObj.getBlock(x, y + tmp.heightFromGround,
								z) == Blocks.air) {
		...
		doGen = TerrainGen.populate(p_73153_1_, worldObj, rand, p_73153_2_,
				p_73153_3_, flag, ICE);
		for (k1 = 0; doGen && k1 < 16; ++k1) {
			for (l1 = 0; l1 < 16; ++l1) {
				...
				if (this.worldObj.func_147478_e(k1 + k, i2, l1 + l, true)) {
					...
					for (int i = -1; i <= 1; i++) {
						for (int j = -1; j <= 1; j++) {
							diff += this.worldObj.getPrecipitationHeight(k1 + k
									+ i, l1 + l + j)
									- i2;
						}
					}
					...
					this.worldObj.setBlock(k1 + k, i2, l1 + l,
							ACBlocks.frostSnow,
							MathHelper.clamp_int(diff, -3, 3) + 3, 2);
				}
			}
		}
